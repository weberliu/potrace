...
function calcSums(path) {
  ...
  path.sums = [];
  var s = path.sums;
  s.push(new _Sum2.default(0, 0, 0, 0, 0));
  for (i = 0; i < path.points.length; i++) {
    x = path.points[i].x - path.x0;
    y = path.points[i].y - path.y0;
    s.push(new _Sum2.default(s[i].x + x, s[i].y + y, s[i].xy + x * y, s[i].x2 + x * x, s[i].y2 + y * y));
  }
}
...
function calcLon(path) {
  ...
  var constraint = [new _Point2.default(), new _Point2.default()];
  var cur = new _Point2.default();
  var off = new _Point2.default();
  var dk = new _Point2.default();
  var foundk;
  ...
  for (i = n - 1; i >= 0; i--) {
    ...
    while (1) {
      foundk = 0;
      dir = (3 + 3 * (0, _utils.sign)(pt[k].x - pt[k1].x) + (0, _utils.sign)(pt[k].y - pt[k1].y)) / 2;
      ct[dir]++;
    ...
    if (foundk === 0) {
      dk.x = (0, _utils.sign)(pt[k].x - pt[k1].x);
      dk.y = (0, _utils.sign)(pt[k].y - pt[k1].y);
...
function bestPolygon(path) {
  function penalty3(path, i, j) {
    ...
    var pt = path.points;
    var sums = path.sums;
    var x, y, xy, x2, y2, k, a, b, c, s, px, py, ex, ey;
    var r = 0;
    if (j >= n) {
      j -= n;
      r = 1;
...
function adjustVertices(path) {
  function pointslope(path, i, j, ctr, dir) {
    var n = path.points.length;
    var sums = path.sums;
    var x, y, x2, xy, y2, k, a, b, c, lambda2, l;
    var r = 0;
  ...
  var m = path.m,
      ...
      k,
      l,
      s = new _Point2.default();

  path.curve = new _Curve2.default(m);

  for (i = 0; i < m; i++) {
    j = po[mod(i + 1, m)];
    j = mod(j - po[i], n) + po[i];
    ctr[i] = new _Point2.default();
    dir[i] = new _Point2.default();
    pointslope(path, po[i], j, ctr[i], dir[i]);
  }

  for (i = 0; i < m; i++) {
    q[i] = new _Quad2.default();
  ...
  for (i = 0; i < m; i++) {
    Q = new _Quad2.default();
    w = new _Point2.default();
    ...
    dy = Math.abs(w.y - s.y);
    if (dx <= 0.5 && dy <= 0.5) {
      path.curve.vertex[i] = new _Point2.default(w.x + x0, w.y + y0);
      continue;
    }
    ...
    path.curve.vertex[i] = new _Point2.default(xmin + x0, ymin + y0);
  }
}
...
function optiCurve(path, info) {
  function Opti() {
    this.pen = 0;
    this.c = [new _Point2.default(), new _Point2.default()];
    this.t = 0;
    this.s = 0;
    this.alpha = 0;
  }
  ...
  function opti_penalty(path, i, j, res, opttolerance, convc, areac) {
    ...
    for (k = k1; k != j; k = k1) {
      ...
      if ((0, _utils.sign)(cprod(vertex[i], vertex[i1], vertex[k1], vertex[k2])) != conv) {
        return 1;
      }
  ...
  for (i = 0; i < m; i++) {
    if (curve.tag[i] == 'CURVE') {
      convc[i] = (0, _utils.sign)(dpara(vert[mod(i - 1, m)], vert[i], vert[mod(i + 1, m)]));
    } else {
      convc[i] = 0;
    }
  }
